...
<body>
    ...
    ?>
        <div class="d-flex flex-column text-center justify-content-center">
            ...
            <form action="./?mode=default">
                <input class="input-group-submit p-2 w-50 mx-auto mt-3 cursor-pointer" type="submit" name="return" value="Go again">
            </form>
        </div>
    <?php
    } else {
    ?>
        ...
        <form class="d-flex flex-column text-center align-items-center">
            <div class="btn-group w-50" role="group" aria-label="Mode selection">
            ...
            switch ($_GET['mode']) {
                case "default":
            ?>
                    <span class="mt-4 w-50">Download the wallpaper from dirkdev.com. The image can be changed at any time at https://wallpaper.dirkdev.com/manage/{username}</span>
                    <input required autocomplete="off" id="name-input-field" class="input-group-text form-control p-2 w-50 mt-4" type="text" name="username" placeholder="Username">
                ...
                case "url":
                ?>
                    <span class="mt-4 w-50">Download the wallpaper from a url of your own. Link to an image you found on the internet or use a link of which you can change the image contents.</span>
                    <input required autocomplete="off" id="url-input-field" class="input-group-text form-control p-2 w-50 mt-4" type="text" name="url" placeholder="URL">
                <?php
                    break;
                case "image":
                ?>
                    <span class="mt-4 w-50">Pick an image. Because this image doesn't have to be downloaded, this will work even on devices without an internet connection</span>
                    <input type="file" id="wallpaper" name="wallpaper" class="mt-4">
                <?php
                    break;
                case "xkcd":
                ?>
                    <span class="mt-4 w-50">Every time the user starts their computer or changes their background, a random comic from https://xkcd.com will be downloaded and set as desktop wallpaper</span>
            <?php
                    break;
            }
            ?>
            <input class=" input-group-submit p-2 w-50 mt-4 cursor-pointer" type="submit" name="submit" value="Download">
        </form>
    <?php
    }
    ?>
